...
function applyInstanceSpecificShadowDomListeners(selector) {
  ...
  if (datepickers.some(function (dp) { return dp.parentElement === parent })) return
  events.forEach(function (event) { parent.addEventListener(event, shadowDomHandler) })
}
...
function remove() {
  ...
  var el = this.el
  var sibling = this.sibling
  var isInShadowDom = this.isInShadowDom
  var _this = this
